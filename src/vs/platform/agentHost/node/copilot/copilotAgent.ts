...
export class CopilotAgent extends Disposable implements IAgent {
	...
	getProtectedResources(): ProtectedResourceMetadata[] {
		...
		// Upstream also allows signed-out operation when `allowSignedOutWhenUsable`
		// is enabled and BYOK models are present — honor that opt-out even when
		// Nika GitHub is enabled.
		...
		return [
			githubRequired && !(allowSignedOutWhenUsable && this._byokModels.length > 0) ? { ...copilotResource, required: true } : { ...copilotResource, required: false },
