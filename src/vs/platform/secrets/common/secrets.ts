...
export class BaseSecretStorageService extends Disposable implements ISecretStorageService {
	 ...
	 * `github.auth` through `APPLICATION_SHARED` moved the token into a separate
	 * shared-storage database with no copy left behind, making the GitHub login
	 * unrecoverable if that database is ever missing or corrupt.)
	 */
