...
- **CLI / Claude** sessions return registered language models whose `targetChatSessionType` matches the session type, plus **See general-pool** models (`vendor === 'nika'` with no `targetChatSessionType`). See models are provided by the built-in extension and are not scoped to a session type, so they are surfaced explicitly so they can be picked and sent from Copilot Chat sessions. CLI sessions are agent-host backed, so their `nika/...` selections are forwarded at send time only when the agent host's BYOK bridge has mirrored the model (`chat.agentHost.byokModels.enabled`, default ON in SeeCode); `AgentHostSessionHandler` then routes them through the bridge instead of dropping them as foreign.
