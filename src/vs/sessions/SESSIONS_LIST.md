...
| See + current workspace | On | `sessions.list.showOnlyNikaAndCurrentWorkspace` (See fork): keeps only Copilot Chat provider sessions (which host the See BYOK models) scoped to the current window's workspace folder, hiding sessions from other apps (e.g. Codex) and from other workspaces. The setting key lives in `workbench/contrib/chat/common/chatSessionsService.ts` and is shared with the workbench's Copilot Chat sessions view (`AgentSessionsControl`, which applies the same provider + workspace predicate via `AgentSessionsDataSource`'s `extraExclude`). Here the workspace is the window's opened folder (`openWindowSourceFolder`, set when opening via the folder picker) falling back to the first folder of the workspace context service; in the workbench it is the workspace context service's folders compared against session `workingDirectoryPath`/`repositoryPath`/`worktreePath` metadata. When no workspace folder is available, only the provider (See) filter is applied. |
