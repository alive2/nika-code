			...
			if (message.role === vscode.LanguageModelChatMessageRole.System) {
				...
				for (const part of message.content) {
					if (part instanceof vscode.LanguageModelToolResultPart2 || part instanceof vscode.LanguageModelToolResultPart) {
						...
						// The agent runtime sends attached PDFs to language-model providers as DataParts.
						// Preserve them as raw Document parts; dropping them here makes BYOK providers
						// receive an empty attachment list even though the chat UI shows the PDF.
						chatMessages.push(
							<UserMessage priority={0}>
								<Document data={Buffer.from(part.data).toString('base64')} mediaType='application/pdf' />
							</UserMessage>
						);
