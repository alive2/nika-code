...
import { NIKA_DEEPSEEK_SECRET, NIKA_GEMINI_SECRET } from './nikaModels';
...
export class NikaSettingsEditor extends Disposable {
	...
	private async _state(): Promise<Record<string, unknown>> {
		...
		return {
			...
			settings: {
				defaultModel: value('defaultModel', 'nika/deepseek-v4-flash'),
				...
				'agent.plan': value('agent.plan', 'nika/deepseek-v4-pro'),
				'agent.explore': value('agent.explore', 'nika/deepseek-v4-flash'),
				'agent.utility': value('agent.utility', 'nika/deepseek-v4-flash'),
				'agent.utilitySmall': value('agent.utilitySmall', 'nika/gemini-2.5-flash-lite'),
				'agent.inlineChat': value('agent.inlineChat', 'nika/deepseek-v4-flash'),
	...
	private async _applyRecommendedAgents(): Promise<void> {
		const config = vscode.workspace.getConfiguration('nika');
		const recommended: Record<string, string> = {
			'agent.plan': 'nika/deepseek-v4-pro',
			'agent.explore': 'nika/deepseek-v4-flash',
			'agent.utility': 'nika/deepseek-v4-flash',
			'agent.utilitySmall': 'nika/gemini-2.5-flash-lite',
			'agent.inlineChat': 'nika/deepseek-v4-flash',
		};
		await Promise.all(Object.entries(recommended).map(([key, value]) => config.update(key, value, vscode.ConfigurationTarget.Global)));
		await Promise.all([
			vscode.workspace.getConfiguration('chat').update('planAgent.defaultModel', this._qualifiedAgentModel(recommended['agent.plan']), vscode.ConfigurationTarget.Global),
			vscode.workspace.getConfiguration('chat').update('exploreAgent.defaultModel', this._qualifiedAgentModel(recommended['agent.explore']), vscode.ConfigurationTarget.Global),
			vscode.workspace.getConfiguration('chat').update('utilityModel', recommended['agent.utility'], vscode.ConfigurationTarget.Global),
			vscode.workspace.getConfiguration('chat').update('utilitySmallModel', recommended['agent.utilitySmall'], vscode.ConfigurationTarget.Global),
			vscode.workspace.getConfiguration('inlineChat').update('defaultModel', this._qualifiedAgentModel(recommended['agent.inlineChat']), vscode.ConfigurationTarget.Global),
		]);
	...
	async checkForUpdates(showNoUpdate: boolean): Promise<void> {
		try {
			...
			if (compareVersions(release.tag_name, current) > 0) {
				...
				const choice = await vscode.window.showInformationMessage(vscode.l10n.t('Nika Code {0} is available. You have {1}.', release.tag_name, current), view);
				if (choice === view && release.html_url) {
					await vscode.env.openExternal(vscode.Uri.parse(release.html_url));
				}
			} else if (showNoUpdate) {
				void vscode.window.showInformationMessage(vscode.l10n.t('Nika Code is up to date.'));
			}
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			this.log('ERROR', vscode.l10n.t('Release check failed: {0}', message));
			if (showNoUpdate) { void vscode.window.showErrorMessage(vscode.l10n.t('Could not check for Nika Code updates: {0}', message)); }
		}
	}
...
.brand{display:flex;align-items:center;gap:10px;font-size:18px;font-weight:650;margin:0 8px 28px}.mark{width:26px;height:26px;border-radius:8px;background:linear-gradient(135deg,#8b5cf6,#06b6d4);box-shadow:0 0 24px color-mix(in srgb,#8b5cf6 45%,transparent)}
...
</style></head><body><div class="shell"><aside class="side"><div class="brand"><span class="mark"></span>Nika</div><nav>
${[['overview', vscode.l10n.t('Overview')], ['models', vscode.l10n.t('Models')], ['vision', vscode.l10n.t('Vision')], ['pdf', vscode.l10n.t('PDF')], ['agents', vscode.l10n.t('Agents')], ['diagnostics', vscode.l10n.t('Diagnostics')]].map(([id, label], i) => `<button class="${i === 0 ? 'active' : ''}" data-section="${id}">${label}</button>`).join('')}
</nav></aside><main>
<section id="overview" class="active"><h1>${vscode.l10n.t('Nika Settings')}</h1><p class="lead">${vscode.l10n.t('Native DeepSeek, Gemini, Gemma, vision, and PDF support for Nika Code.')}</p>
<div class="card"><div class="row"><label><strong>${vscode.l10n.t('DeepSeek')}</strong><span class="hint">${vscode.l10n.t('Flash, Pro, and experimental Responses')}</span></label><span id="deepseek-status"></span></div><div class="row"><label><strong>${vscode.l10n.t('Gemini')}</strong><span class="hint">${vscode.l10n.t('Flash and Flash Lite')}</span></label><span id="gemini-status"></span></div><div class="row"><label><strong>${vscode.l10n.t('Ollama')}</strong><span class="hint">${vscode.l10n.t('Gemma 4 31B at the configured host')}</span></label><span id="ollama-status"></span></div></div>
<div class="card"><div class="row"><label><strong>${vscode.l10n.t('Nika Code version')}</strong></label><span id="app-version"></span></div><div class="row"><label><strong>${vscode.l10n.t('Bundled Copilot version')}</strong></label><span id="extension-version"></span></div></div></section>
...
<section id="vision"><h1>${vscode.l10n.t('Vision')}</h1><p class="lead">${vscode.l10n.t('DeepSeek receives cached textual image descriptions. Gemini and Gemma receive images directly.')}</p><div class="card">
${this._secretRow('deepseek', vscode.l10n.t('DeepSeek API key'))}${this._secretRow('gemini', vscode.l10n.t('Gemini API key'))}${this._selectRow('visionModel', vscode.l10n.t('Image-description backend'), [['gemini-2.5-flash','Gemini 2.5 Flash'],['gemini-2.5-flash-lite','Gemini 2.5 Flash Lite'],['gemma4:31b','Gemma 4 31B (Ollama)'],['vscode',vscode.l10n.t('Another VS Code vision model')]])}${this._textRow('visionVSCodeModel', vscode.l10n.t('VS Code vision model identifier'))}${this._textRow('ollamaBaseUrl', vscode.l10n.t('Ollama host'))}
</div></section>
...
<section id="agents"><h1>${vscode.l10n.t('Agents')}</h1><p class="lead">${vscode.l10n.t('Assign Nika models to built-in roles. Inline Chat is applied to the native inlineChat.defaultModel setting.')}</p><div class="card">${['plan','explore','utility','utilitySmall','inlineChat'].map(id=>this._textRow(`agent.${id}`, ({plan:vscode.l10n.t('Plan'),explore:vscode.l10n.t('Explore'),utility:vscode.l10n.t('Utility'),utilitySmall:vscode.l10n.t('Utility Small'),inlineChat:vscode.l10n.t('Inline Chat')} as Record<string,string>)[id])).join('')}</div><div class="actions"><button class="action" data-action="recommendedAgents">${vscode.l10n.t('Apply recommended mappings')}</button></div></section>
...
function status(id,configured){const result=state.connections[id];const text=result?(result.ok?${JSON.stringify(vscode.l10n.t('Connected'))}:result.message):(configured?${JSON.stringify(vscode.l10n.t('Configured'))}:${JSON.stringify(vscode.l10n.t('Not configured'))});document.getElementById(id+'-status').innerHTML='<span class="pill '+(result?.ok?'ok':'')+'"><span class="dot"></span></span> ';document.getElementById(id+'-status').append(document.createTextNode(text));}status('deepseek',state.deepseekConfigured);status('gemini',state.geminiConfigured);status('ollama',true);
...
document.querySelectorAll('nav button').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('nav button,section').forEach(el=>el.classList.remove('active'));button.classList.add('active');document.getElementById(button.dataset.section).classList.add('active');}));
