...
describe('NikaLlamaCppProvider', () => {
	...
	it('uses meta.n_ctx as the served context window when the server reports it', async () => {
		const { provider } = createProvider({
			json: {
				object: 'list',
				data: [{
		...
		expect(model).toBeDefined();
		// `meta.n_ctx` is the served context (from `--ctx-size`); the much
		// larger `n_ctx_train` must NOT be used.
	...
	it('falls back to the default window when meta.n_ctx is absent or invalid', async () => {
		const { provider } = createProvider({
			json: {
				object: 'list',
				data: [
					{ id: 'unloaded', object: 'model', created: 1, owned_by: 'llamacpp' },
					{ id: 'broken', object: 'model', created: 2, owned_by: 'llamacpp', meta: { n_ctx: 'lots' } },
					{ id: 'zero', object: 'model', created: 3, owned_by: 'llamacpp', meta: { n_ctx: 0 } },
				],
			},
		});
		const catalog = await provider.getCatalog('http://localhost:8080');
